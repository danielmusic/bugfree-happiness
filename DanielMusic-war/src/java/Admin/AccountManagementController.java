    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            switch (target) {
                ...
                case "DeleteAccount":
                    if (checkLogin(response)) {
                        returnHelper = accountManagementBean.deleteAccount(Long.parseLong(id));
                        if (returnHelper.getResult()) {
                            if (source != null && source.equals("artistManagement")) {
                                List<Artist> artists = adminManagementBean.listAllArtists(true);
                                if (artists == null) {
                ...
                case "GenerateEmail":
                    if (checkLogin(response)) {
                        ...
                        if (returnHelper.getResult()) {
                            nextPage = "/admin/workspace.jsp?goodMsg=" + returnHelper.getDescription();
                        } else {
                            nextPage = "/admin/workspace.jsp?errMsg=" + returnHelper.getDescription();
                        }
                    } else {
                        response.sendRedirect("/admin/login.jsp?errMsg=Session Expired.");
                        return;
