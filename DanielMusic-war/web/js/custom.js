...
jQuery(document).ready(function($) {
	...
	function after_page_load(container) {
		...
		/* Contact Form
	 	...
	 	if ( $( '.contact-form' ).length ) {
			...
			$form.append('<div id="ajax-message" style="display:none;margin-top:20px;"></div>');
		...
		setTimeout( function(){ $( '.masonry', container ).isotope( 'layout' ) }, 1000);
	...
	(function() {
		...
		$( 'body' ).on( 'change', '.ready-filter', function(){
			...
			setTimeout( function(){ $( '#'+ID ).isotope( 'layout' ) }, 500);
		...
		$(window).on( 'resize', function(){
    		setTimeout( function(){ $( '.masonry' ).isotope( 'layout' ) }, 500);
		});
