...
(function (a) {
    a.ScampPlayer = function (b, c) {
                ...
                onfinish: function () {
                    ...
                    if (d.settings.loop) {
                        ...
                        if (w != d.settings.tracklist_length) {
                            ...
                            var mp3URL = d.settings.track.find(".sp-track-control").attr("href");

                            d.grabTrack(pageURL, mp3URL);

                            //return mp3 url
                            //alert(d.settings.track.find(".sp-track-control").attr("href"));
                            //return artist name
                            //alert(d.settings.track.find(".sp-artist-button").text());
                            //return #!/page
        ...
        //custom code
        d.grabTrack = function (pageURL, mp3URL) {
            ...
            $.ajax({
                ...
                url: url,
                dataType: "text",
                data: {'pageURL': pageURL, 'mp3URL': mp3URL},
                success: function (val) {
                    var json = JSON.parse(val);
                    var track = {
                        artist: json.artist,
                        title: json.title,
                        url: 'http://sounds.sg.storage.googleapis.com/' + json.url,
                        cover: 'http://sounds.sg.storage.googleapis.com/' + json.cover,
                        artist_url: "javascript:_artistLookup(" + json.artistID + ")",
                        shop_url: "javascript:_addTrackToCart(" + json.musicID + ")"
                    };
                    d.addTrack(track, 'play_tracks');
                },
        ...
        _addTrackToCart = function (trackID) {
